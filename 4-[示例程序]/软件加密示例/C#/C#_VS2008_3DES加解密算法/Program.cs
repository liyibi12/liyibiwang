...
namespace ViKeySample
{
    public class EncryptHelper
    {
        ...
        public static string Encrypt3DES(string a_strString, string a_strKey)
        {
            ...
            return ToHexString(DESEncrypt.TransformFinalBlock(Buffer, 0, Buffer.Length), 8);
        }

        public static string Decrypt3DES(string strString, string a_strKey)
        {
            try
            {
                TripleDESCryptoServiceProvider DES = new TripleDESCryptoServiceProvider();
                ...
                byte[] Buffer = GetBytes(strString);
    ...
    class Program
    {
        ...
        static void ViKeySample()
        {
            ...
            for (j = 0; j < Count; j++)
            {
                ...
                Vikey3DesSetKey(0, ToByte(str1), 0);
                ...
                System.Console.WriteLine("Des加密结果1：" + ToHexString(szDesResult, 8));


                string aaaa = EncryptHelper.Encrypt3DES(strDesText, str1);
                System.Console.WriteLine("Des加密结果2：" + aaaa);

                string bbbb = EncryptHelper.Decrypt3DES(aaaa, str1);
                System.Console.WriteLine("Des解密密结果2：" + bbbb);
